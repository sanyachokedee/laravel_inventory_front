<template>
      ...
      <aside class="z-20 flex-shrink-0 hidden w-64 overflow-y-auto bg-white dark:bg-gray-800 xl:block">
        <div class="py-4 text-gray-500 dark:text-gray-400">
          ...
          <!-- <ul> // เอาออกไปใช้ในส่วนของ Navbar แทน
            <li class="relative px-6 py-3">
      ...
      <!-- Mobile sidebar -->
      <!-- Backdrop -->
      <aside :class="{ 'hidden': this.$store.state.showSideMenu }"  class="fixed inset-y-0 z-20 flex-shrink-0 w-64 mt-16 overflow-y-auto bg-gray-100 dark:bg-gray-800 xl:hidden">
        <div class="py-4 text-gray-500 dark:text-gray-400">
          ...
          <ul class="mt-6">
            <li class="relative px-6 py-3">
              <span
                class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg"
                aria-hidden="true"></span>
                <router-link to="/backend/" class="inline-flex items-center w-full text-sm font-semibold text-gray-800 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 dark:text-gray-100" href="#">
                  <svg
                    class="w-5 h-5"
                    aria-hidden="true"
                    fill="none"
          ...
          <ul>
            <li class="relative px-6 py-3">
              <router-link to ="/backend/products" class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200">
                <svg
                  class="w-5 h-5"
                  aria-hidden="true"
                  fill="none"
